...
class ObjectLabels(ObjectLabelBase):
    ...
    def get_labels_as_tensors(self, format_: str = 'yolox') -> th.Tensor:
        ...
        if format_ == 'yolox':
            # Keep the dataset's six-column label representation intact.
    ...
    def get_labels_as_batched_tensor(obj_label_list: List[ObjectLabels], format_: str = 'yolox') -> th.Tensor:
        ...
        # A dense COCO-style dataset legitimately has a batch in which every
        # image is background.  YOLOX represents this with one all-zero
        # padded target row, rather than dropping the batch (which would
        # silently remove objectness-negative supervision).
        max_num_labels_per_object_frame = max(1, max(len(x) for x in obj_label_list))
